 ...
 * (extended by bdcampbell.net)
 */
//bdc: add texture processing variables (global for now)
var VRMLLoader_UVs_array = [];
var VRMLLoader_has_texture = false;
var VRMLLoader_texture = "";
...
THREE.VRMLLoader.prototype = {
	...
	parse: function ( data ) {
		...
		var parseV2 = function ( lines, scene ) {
			...
			var float2_pattern = /\s+([\d|\.|\+|\-|e]+),?\s+([\d|\.|\+|\-|e]+)/;
				...
				if ( /Transform/.exec( data.string ) || /Group/.exec( data.string ) ) {
					...
					if ( /Box/.exec( data.string ) ) {
                        ...
                        for (var i = 0, j = data.children.length; i < j; i++) {
							...
							//bdc: add Texture UV Coordinates processing
							if ( /TextureCoordinate/.exec (child.string)) {
								VRMLLoader_has_texture = true;
								VRMLLoader_UVs_array = [];
                                for (var k = 0, l = child.children.length; k < l; k++) {
                                    var point = child.children[k];
                                    if (null != (result = float2_pattern.exec(point))) {
                                        vec = new THREE.Vector2(
                                            parseFloat(result[1]),
                                            parseFloat(result[2])
                                        );
                                        VRMLLoader_UVs_array.push( vec );
                                    }
                                }
                                //console.log(VRMLLoader_UVs_array);
                            }
                            ...
                            while ( isRecordingCoordinates && null != (coordIndex = regex.exec(child) ) ) {
                                ...
                                if (points.length >= 3) {
									...
									//bdc: add Texture UV mapping from coordinates
									if(VRMLLoader_has_texture==true) {
										geometry.faceVertexUvs[0].push([ VRMLLoader_UVs_array[points[0]], VRMLLoader_UVs_array[points[skip + 1]], VRMLLoader_UVs_array[points[skip + 2]] ]);
									}
                                }
                            }
                        ...
                        console.log('computing properties of geometry...');
					...
					for ( var i = 0; i < data.children.length; i ++ ) {
						...
						//bdc: add texture URL handling
						if ( /ImageTexture/.exec( child.string ) ) {
							VRMLLoader_has_texture=true;
							for ( var j = 0; j < child.children.length; j ++ ) {
								var parameter = child.children[ j ];
								console.log("parameter: " + parameter);
								var url_value=" " + parameter;
								console.log("|" + url_value.substring(url_value.indexOf("url") + 5, url_value.indexOf(".") + 4) + "|");
								VRMLLoader_texture = new THREE.ImageUtils.loadTexture(url_value.substring(url_value.indexOf("url") + 5, url_value.indexOf(".") + 4));
								parent.material.map = VRMLLoader_texture;
							}
						}
					}
					return;
